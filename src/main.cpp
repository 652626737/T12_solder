...
void TipScreen();
void TempScreen();
float InputFloatScreen(const char* title, float minVal, float maxVal);
void PIDTuningScreen();
void TimerScreen();
...
// EEPROM identifier
#define EEPROM_IDENT   0xE76E   // to identify if EEPROM was written by this program (changed for language support)
...
uint8_t   currentLanguage = LANGUAGE_ENGLISH;  // 0: English, 1: Chinese
...
// Menu items - Chinese language support
...
const char *TempItems[]        = { "温度设置", "默认温度", "休眠温度",
                                   "增压温度", "返回" };
const char *TimerItems[]       = { "定时设置", "休眠定时", "关闭定时",
                                   "增压定时", "返回" };
const char *ControlTypeItems[] = { "控制类型", "直接控制", "PID" };
const char *PIDTuningItems[]   = { "PID 调优", "激进 Kp", "激进 Ki", "激进 Kd",
                                   "保守 Kp", "保守 Ki", "保守 Kd", "返回" };
const char *MainScreenItems[]  = { "主屏幕", "大数字", "更多信息" };
const char *LanguageItems[]    = { "语言", "英语", "中文" };
...
const char *BoostTempItems[]   = { "增压温度", "摄氏度" };
const char *SleepTimerItems[]  = { "休眠定时", "分钟" };
const char *OffTimerItems[]    = { "关闭定时", "分钟" };
const char *BoostTimerItems[]  = { "增压定时", "秒" };
const char *DeleteMessage[]    = { "警告", "不能删除", "最后一个", "烙铁头！" };
const char *MaxTipMessage[]    = { "警告", "已达到", "最大数量", "烙铁头！" };
...
void getEEPROM() {
  ...
  if (identifier == EEPROM_IDENT) {
    ...
    currentLanguage = EEPROM.read(13);
    CurrentTip  =  EEPROM.read(14);
    NumberOfTips = EEPROM.read(15);

    // Read PID parameters (6 floats, 4 bytes each)
    uint16_t pidAddr = 16;
    EEPROM.get(pidAddr, aggKp); pidAddr += sizeof(float);
    EEPROM.get(pidAddr, aggKi); pidAddr += sizeof(float);
    EEPROM.get(pidAddr, aggKd); pidAddr += sizeof(float);
    EEPROM.get(pidAddr, consKp); pidAddr += sizeof(float);
    EEPROM.get(pidAddr, consKi); pidAddr += sizeof(float);
    EEPROM.get(pidAddr, consKd); pidAddr += sizeof(float);

    uint8_t i, j;
    uint16_t counter = 16 + 6 * sizeof(float); // 16 + 24 = 40
...
void updateEEPROM() {
  ...
  EEPROM.update(13, currentLanguage);
  EEPROM.update(14, CurrentTip);
  EEPROM.update(15, NumberOfTips);

  // Write PID parameters (6 floats, 4 bytes each)
  uint16_t pidAddr = 16;
  EEPROM.put(pidAddr, aggKp); pidAddr += sizeof(float);
  EEPROM.put(pidAddr, aggKi); pidAddr += sizeof(float);
  EEPROM.put(pidAddr, aggKd); pidAddr += sizeof(float);
  EEPROM.put(pidAddr, consKp); pidAddr += sizeof(float);
  EEPROM.put(pidAddr, consKi); pidAddr += sizeof(float);
  EEPROM.put(pidAddr, consKd); pidAddr += sizeof(float);
...
void MainScreen() {
  ...
  do {
    ...
    u8g.setFontPosTop();
    u8g.drawStr( 0, 0,  F("设定:"));
    u8g.setPrintPos(40,0);
    u8g.print(Setpoint, 0);

    // draw status of heater
    u8g.setPrintPos(83,0);
    if (ShowTemp > 500)    u8g.print(F("错误"));
    else if (inOffMode)    u8g.print(F("关闭"));
    else if (inSleepMode)  u8g.print(F("休眠"));
    else if (inBoostMode)  u8g.print(F("增压"));
    else if (isWorky)      u8g.print(F("工作"));
    else if (Output < 180) u8g.print(F("加热"));
    else                   u8g.print(F("保温"));
...
void SetupScreen() {
  ...
  while (repeat) {
    ...
    switch (selection) {
      ...
      case 4:   PIDTuningScreen(); break;
      case 5:   MainScrType = MenuScreen(MainScreenItems, sizeof(MainScreenItems), MainScrType); break;
      case 6:   currentLanguage = MenuScreen(LanguageItems, sizeof(LanguageItems), currentLanguage); break;
...
// float input screen for PID parameters (value * 10 stored as integer)
float InputFloatScreen(const char* title, float minVal, float maxVal) {
  uint16_t valueInt;
  bool lastbutton = (!digitalRead(BUTTON_PIN));

  // valueInt is stored as value * 10
  do {
    valueInt = getRotary();
    float valueFloat = valueInt / 10.0;
    u8g.firstPage();
    do {
      u8g.setFont(u8g_font_9x15);
      u8g.setFontPosTop();
      ...
      u8g.print(valueFloat, 1);
    } while(u8g.nextPage());
    if (lastbutton && digitalRead(BUTTON_PIN)) {delay(10); lastbutton = false;}
  } while (digitalRead(BUTTON_PIN) || lastbutton);

  beep();
  return valueInt / 10.0;
}


// PID tuning settings screen
void PIDTuningScreen() {
  uint8_t selection = 0;
  bool repeat = true;
  while (repeat) {
    selection = MenuScreen(PIDTuningItems, sizeof(PIDTuningItems), selection);
    switch (selection) {
      case 0:   // 激进 Kp
                setRotary(0, 500, 5, (int)(aggKp * 10));
                aggKp = InputFloatScreen("激进 Kp:", 0, 50.0);
                break;
      case 1:   // 激进 Ki
                setRotary(0, 200, 2, (int)(aggKi * 10));
                aggKi = InputFloatScreen("激进 Ki:", 0, 20.0);
                break;
      case 2:   // 激进 Kd
                setRotary(0, 200, 2, (int)(aggKd * 10));
                aggKd = InputFloatScreen("激进 Kd:", 0, 20.0);
                break;
      case 3:   // 保守 Kp
                setRotary(0, 500, 5, (int)(consKp * 10));
                consKp = InputFloatScreen("保守 Kp:", 0, 50.0);
                break;
      case 4:   // 保守 Ki
                setRotary(0, 200, 2, (int)(consKi * 10));
                consKi = InputFloatScreen("保守 Ki:", 0, 20.0);
                break;
      case 5:   // 保守 Kd
                setRotary(0, 200, 2, (int)(consKd * 10));
                consKd = InputFloatScreen("保守 Kd:", 0, 20.0);
                break;
      default:  repeat = false; break;
    }
  }
...
// menu screen
uint8_t MenuScreen(const char *Items[], uint8_t numberOfItems, uint8_t selected) {
  bool isTipScreen = (strcmp(Items[0], "烙铁头:") == 0);
  ...
  do {
    ...
    u8g.firstPage();
      do {
        u8g.setFont(u8g_font_9x15);
        u8g.setFontPosTop();
        u8g.setPrintPos(0,  0); u8g.print(F("固件: ")); u8g.print(VERSION);
        u8g.setPrintPos(0, 16); u8g.print(F("温度: "));  u8g.print(fTmp, 1); u8g.print(F(" °C"));
        u8g.setPrintPos(0, 32); u8g.print(F("输入电压:"));  u8g.print(fVin, 1); u8g.print(F("V"));
        u8g.setPrintPos(0, 48); u8g.print(F("电源电压:")); u8g.print(fVcc, 1); u8g.print(F("V"));
      } while(u8g.nextPage());
    if (lastbutton && digitalRead(BUTTON_PIN)) {delay(10); lastbutton = false;}
  } while (digitalRead(BUTTON_PIN) || lastbutton);
  ...
  for (uint8_t CalStep = 0; CalStep < 3; CalStep++) {
    ...
    do {
      ...
      do {
        u8g.setFont(u8g_font_9x15);
        u8g.setFontPosTop();
        u8g.drawStr( 0, 0,  F("校准"));
        u8g.setPrintPos(0, 16); u8g.print(F("步骤: ")); u8g.print(CalStep + 1); u8g.print(F(" / 3"));
        if (isWorky) {
          u8g.setPrintPos(0, 32); u8g.print(F("设定测量"));
          u8g.setPrintPos(0, 48); u8g.print(F("温度：")); u8g.print(getRotary());
        } else {
          u8g.setPrintPos(0, 32); u8g.print(F("ADC 值：")); u8g.print(uint16_t(RawTemp));
          u8g.setPrintPos(0, 48); u8g.print(F("请稍候..."));
        }
      } while(u8g.nextPage());
    if (lastbutton && digitalRead(BUTTON_PIN)) {delay(10); lastbutton = false;}
    } while (digitalRead(BUTTON_PIN) || lastbutton);
...
// 在指定位置绘制一个中文字符（16x16 点阵）
void drawChineseChar(uint8_t x, uint8_t y, uint16_t unicode) {
  const uint8_t *charData = find_chinese_char(unicode);
  if (charData == NULL) return;

  u8g.drawStr(x, y, " ");  // 占位，设置位置

  // U8Glib 不支持直接绘制自定义 bitmap，使用 drawBox 逐点绘制效率太低
  // 改用字符串方式：仅当字符在内置字体中时显示
  // 对于中文，我们需要使用低级别像素操作

  for (uint8_t i = 0; i < 16; i++) {
    uint8_t byte1 = pgm_read_byte(&charData[i * 2]);
    uint8_t byte2 = pgm_read_byte(&charData[i * 2 + 1]);
    for (uint8_t bit = 7; bit <= 0; bit--) {
      if (byte1 & (1 << bit)) u8g.drawPixel(x + (7 - bit), y + i);
    }
    for (uint8_t bit = 7; bit <= 0; bit--) {
      if (byte2 & (1 << bit)) u8g.drawPixel(x + 8 + (7 - bit), y + i);
    }
  }
}
